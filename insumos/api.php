...
function actionChamadoInfo(array $me): void {
    ...
    // Detecta a URL base do GLPI dinamicamente — funciona em qualquer instância
    // __DIR__ = /var/www/html/<instancia>/plugins/insumos
    // Sobe 2 níveis para chegar na raiz da instância GLPI
    $glpiRoot = rtrim(str_replace($_SERVER['DOCUMENT_ROOT'], '', dirname(__DIR__, 1)), '/');
    $glpiUrl  = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https' : 'http')
              . '://' . $_SERVER['HTTP_HOST'] . $glpiRoot . '/apirest.php';
