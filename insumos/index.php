...
<head>
  ...
  <script>(function(){
    // Se estiver fora do iframe (acesso direto), SEMPRE redireciona para
    // front/main.php que é o ponto de entrada correto dentro do GLPI.
    // Isso garante que na 1ª, 2ª ou qualquer visita o fluxo esteja certo.
    if(window === window.top){
      var _parts = window.location.pathname.split('/plugins/insumos');
      var _glpiBase = _parts[0]; // ex: /ti, /motorista, ou vazio
      window.location.replace(_glpiBase + '/plugins/insumos/front/main.php');
      return;
    }

    // Dentro do iframe: marca o modo embed
    document.documentElement.setAttribute("data-embed","glpi");

    // Captura token SSO vindo da URL (após redirect do sso.php)
  ...
  /* ============================================================
     MODO EMBED GLPI
     Cria barra de abas horizontal igual ao screenshot original:
     "Insumos T.I. | Dashboard | Estoque | Painel..."
  ============================================================ */

  /* Esconde sidebar, overlay, burger e topbar original */
  [data-embed="glpi"] .sidebar,
  [data-embed="glpi"] .sidebar-overlay,
  [data-embed="glpi"] .burger-btn,
  [data-embed="glpi"] .topbar { display: none !important; }

  /* Tab-bar injetada via JS */
  [data-embed="glpi"] #embed-tabbar {
  ...
  [data-embed="glpi"] #embed-tabbar::-webkit-scrollbar { display: none; }

  /* Label "Insumos T.I." */
  [data-embed="glpi"] .etab-label {
  ...
  [data-embed="glpi"] .etab-btn:hover {
    color: #e2e8f0 !important;
    background: rgba(255,255,255,.06) !important;
  }
  [data-embed="glpi"] .etab-btn.active {
    color: #fff !important;
...
<main class="main" id="mainContent">
  ...
  <section class="page-content" id="page-entrada">
    ...
    </div><button type="submit" class="btn btn-primary btn-full mt-16 btn-submit"><span class="spinner"></span> ⬆️ Registrar Entrada</button></form></div>
...
<script>
/* ============================================================
   EMBED GLPI — injeta a tab-bar horizontal quando em iframe
============================================================ */
(function () {
  if (document.documentElement.getAttribute('data-embed') !== 'glpi') return;

  // Abas visíveis na barra (page -> label, icon)
  var TABS = [
    { page: 'dashboard',      icon: '📊', label: 'Dashboard' },
    { page: 'estoque',        icon: '📦', label: 'Estoque',        perm: 'p_estoque' },
    { page: 'painel-estoque', icon: '🗂️', label: 'Painel de Estoque', perm: 'p_estoque' },
    { page: 'entrada',        icon: '⬆️', label: 'Entrada',        perm: 'p_entrada' },
    { page: 'saida',          icon: '⬇️', label: 'Saída',          perm: 'p_saida' },
    { page: 'inventario',     icon: '🔢', label: 'Inventário',     perm: 'p_inventario' },
    { page: 'historico',      icon: '📋', label: 'Histórico',      perm: 'p_historico' },
    { page: 'relatorio',      icon: '📈', label: 'Relatórios',     perm: 'p_relatorio' },
  ];

  function getPerms() {
    try {
      var raw = localStorage.getItem('insumos_perms');
      return raw ? JSON.parse(raw) : null;
    } catch(e) { return null; }
  }

  function buildBar(perms) {
    var bar = document.getElementById('embed-tabbar');
    if (bar) bar.remove();
    ...
    lbl.className = 'etab-label';
    lbl.textContent = '📦 Insumos T.I.';
    bar.appendChild(lbl);

    var currentPage = localStorage.getItem('insumos_current_page') || 'dashboard';

    TABS.forEach(function (tab) {
      // Filtra por permissão
      if (tab.perm && perms && !perms[tab.perm]) return;

      var btn = document.createElement('button');
      btn.className = 'etab-btn' + (tab.page === currentPage ? ' active' : '');
      btn.innerHTML = '<span>' + tab.icon + '</span><span>' + tab.label + '</span>';
      btn.onclick = function () {
        document.querySelectorAll('#embed-tabbar .etab-btn').forEach(function(b){ b.classList.remove('active'); });
        btn.classList.add('active');
        if (typeof navTo === 'function') navTo(tab.page);
        localStorage.setItem('insumos_current_page', tab.page);
      };
      bar.appendChild(btn);
    });

    // Insere a bar antes do <main>
  ...
  // Aguarda a app carregar as permissões e depois constrói a barra
  ...
  // Reconstrói a barra quando navTo é chamado para atualizar aba ativa
  var _origNavTo;
  document.addEventListener('DOMContentLoaded', function () {
    tryBuild(15);

    // Sobrescreve navTo para sincronizar aba ativa
    var navInterval = setInterval(function () {
      if (typeof navTo === 'function' && !window._embedPatched) {
        window._embedPatched = true;
        var orig = navTo;
        window.navTo = function(page) {
          orig(page);
          localStorage.setItem('insumos_current_page', page);
          document.querySelectorAll('#embed-tabbar .etab-btn').forEach(function(b){
            b.classList.remove('active');
          });
          // Encontra o botão da aba pelo data ou texto e ativa
          var tabs = document.querySelectorAll('#embed-tabbar .etab-btn');
          var idx = TABS.findIndex(function(t){ return t.page === page; });
          // Conta abas visíveis para achar o índice correto
          var visible = 0;
          TABS.forEach(function(t, i) {
            if (i === idx) {
              var btn = tabs[visible];
              if (btn) btn.classList.add('active');
            }
            if (!t.perm || !getPerms() || getPerms()[t.perm]) visible++;
          });
        };
